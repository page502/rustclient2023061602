...
Version: %s
Architecture: %s
Maintainer: rustdesk <info@clouddesktop.com>
...
def build_flutter_windows(version, features):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/clouddesktop.exe')
    os.chdir('../..')
    if os.path.exists('./clouddesktop_portable.exe'):
        os.replace('./target/release/clouddesktop-portable-packer.exe',
                   './clouddesktop_portable.exe')
    else:
        os.rename('./target/release/clouddesktop-portable-packer.exe',
                  './clouddesktop_portable.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/clouddesktop_portable.exe')
    os.rename('./clouddesktop_portable.exe', f'./clouddesktop-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/clouddesktop-{version}-install.exe')
...
def main():
    ...
    if windows:
        ...
        # system2('upx.exe target/release/clouddesktop.exe')
        system2('mv target/release/clouddesktop.exe target/release/RustDesk.exe')
        pa = os.environ.get('P')
        if pa:
            system2(
                f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  '
                'target\\release\\clouddesktop.exe')
        else:
            print('Not signed')
        system2(
